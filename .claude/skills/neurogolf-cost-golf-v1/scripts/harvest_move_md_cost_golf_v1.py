#!/usr/bin/env python3
"""Move downloaded markdown write-ups into claude-for-chrome-cost-golf-v1-markdown/.

Use snapshot-diff so shared ~/Downloads files are not touched.

Usage:
  python harvest_move_md_cost_golf_v1.py snap
  python harvest_move_md_cost_golf_v1.py move <task_num> <session_id>
"""
import glob
import os
import shutil
...
SNAP = "/tmp/harvest_md_cost_golf_v1_snap.txt"
DL = os.path.expanduser("~/Downloads")
# repo root = four levels above this script (.../<root>/.claude/skills/<skill>/scripts/)
ROOT = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", "..", ".."))
BASE = f"{ROOT}/claude-for-chrome-cost-golf-v1-markdown"
MAN = f"{ROOT}/zzz_sessions/markdown-harvest-manifest-cost-golf-v1.csv"
